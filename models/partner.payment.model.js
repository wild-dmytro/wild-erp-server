...
const partnerPaymentModel = {
  ...
  /**
   * Отримує статистику платежів
   * @param {Object} options - Опції для фільтрації
   * @param {Date} [options.startDate] - Початкова дата
   * @param {Date} [options.endDate] - Кінцева дата
   * @param {string} [options.currency] - Валюта
   * @returns {Promise<Object>} Статистика платежів
   */
  getPaymentStats: async ({ startDate, endDate, currency } = {}) => {
    const conditions = ["TRUE"];
    const params = [];
    let paramIndex = 1;
    ...
    const query = `
      SELECT
        ...
        SUM(CASE WHEN status = 'hold' THEN 1 ELSE 0 END) as hold_payments,
  ...
  /**
   * Отримує статистику платежів за партнерами
   * @param {Object} options - Опції для фільтрації
   * @param {Date} [options.startDate] - Початкова дата
   * @param {Date} [options.endDate] - Кінцева дата
   * @returns {Promise<Array>} Статистика за партнерами
   */
  getPaymentStatsByPartner: async ({ startDate, endDate } = {}) => {
    const conditions = ["TRUE"];
    const params = [];
    let paramIndex = 1;
    ...
    const query = `
      SELECT 
        ...
        SUM(CASE WHEN pp.status = 'completed' THEN pp.amount ELSE 0 END) as completed_amount,
  ...
  /**
   * Оновлює дані blockchain для платежу
   * @param {number} id - ID платежу
   * @param {Object} blockchainData - Дані blockchain
   * @returns {Promise<Object|null>} Оновлений платіж або null
   */
  updateBlockchainData: async (id, blockchainData) => {
    const {
      transaction_hash,
      block_number,
      gas_used,
    ...
    if (confirmation_date !== undefined) {
      setClauses.push(`confirmation_date = $${paramIndex++}`);
