...
/**
 * Отримання статистики команди за період
 * @param {Object} req - Об'єкт запиту Express
 * @param {Object} res - Об'єкт відповіді Express
 */
exports.getTeamStats = async (req, res) => {
  try {
    ...
    // Перевіряємо права доступу для тімліда
    if (req.user.role === 'teamlead') {
      const currentUser = await teamModel.getTeamLead(req.user.id);
      if (!currentUser || currentUser.team_id !== teamId) {
        return res.status(403).json({
          success: false,
          message: "Доступ заборонено. Ви не є тімлідом цієї команди",
    ...
    if (parsedStartDate && isNaN(parsedStartDate.getTime())) {
      return res.status(400).json({
        success: false,
        message: "Некоректний формат початкової дати",
      });
    }
    if (parsedEndDate && isNaN(parsedEndDate.getTime())) {
      return res.status(400).json({
        success: false,
        message: "Некоректний формат кінцевої дати",
      });
    }
    if (parsedStartDate && parsedEndDate && parsedStartDate > parsedEndDate) {
      return res.status(400).json({
        success: false,
        message: "Початкова дата не може бути пізніше кінцевої",
      });
    }

    // Отримання статистики
    const stats = await teamModel.getTeamStats(teamId, parsedStartDate, parsedEndDate);
    ...
    console.error(`Помилка отримання статистики команди з ID ${req.params.id}:`, err);
    res.status(500).json({
      success: false,
...
/**
 * Отримання всіх команд з детальною інформацією
 * @param {Object} req - Об'єкт запиту Express
 * @param {Object} res - Об'єкт відповіді Express
 */
exports.getTeamsWithDetails = async (req, res) => {
  try {
    // Отримання команд з деталями
    const teams = await teamModel.getTeamsWithDetails();
    ...
    console.error("Помилка отримання команд з деталями:", err);
    res.status(500).json({
      success: false,
      message: "Помилка сервера під час отримання команд з деталями",
    });
  }
};