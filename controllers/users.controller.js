...
/**
 * Призначення користувача до команди
 * @param {Object} req - Об'єкт запиту Express
 * @param {Object} res - Об'єкт відповіді Express
 */
exports.assignUserToTeam = async (req, res) => {
  try {
    ...
    // Призначення користувача до команди
    const result = await userModel.updateUserTeam(userId, team_id);

    res.json({
      success: true,
      message: "Користувача успішно призначено до команди",
      data: result,
    });
  } catch (err) {
    console.error(
      `Помилка призначення користувача з ID ${req.params.id} до команди:`,
      err
    );
    res.status(500).json({
      success: false,
      message: "Помилка сервера під час призначення користувача до команди",
    });
  }
};

/**
 * Призначення користувача до відділу
 * @param {Object} req - Об'єкт запиту Express
 * @param {Object} res - Об'єкт відповіді Express
 */
exports.assignUserToDepartment = async (req, res) => {
  try {
    ...
    // Призначення користувача до відділу
    const result = await userModel.updateUserDepartment(userId, department_id);

    res.json({
      success: true,
      message: "Користувача успішно призначено до відділу",
      data: result,
    });
  } catch (err) {
    console.error(
      `Помилка призначення користувача з ID ${req.params.id} до відділу:`,
      err
    );
    res.status(500).json({
      success: false,
      message: "Помилка сервера під час призначення користувача до відділу",
    });
  }
};
...
/**
 * Отримання всіх користувачів команди
 * @param {Object} req - Об'єкт запиту Express
 * @param {Object} res - Об'єкт відповіді Express
 */
exports.getUsersByTeam = async (req, res) => {
  try {
    ...
    // Перевіряємо, чи є поточний користувач тімлідом цієї команди
    ...
    // Отримання користувачів команди
    ...
    console.error(
      `Помилка отримання користувачів команди з ID ${req.params.teamId}:`,
      err
    );
    res.status(500).json({
      success: false,
      message: "Помилка сервера під час отримання користувачів команди",
    });
  }
};

/**
 * Отримання всіх користувачів відділу
 * @param {Object} req - Об'єкт запиту Express
 * @param {Object} res - Об'єкт відповіді Express
 */
exports.getUsersByDepartment = async (req, res) => {
  try {
    ...
    // Отримання користувачів відділу
    ...
    console.error(
      `Помилка отримання користувачів відділу з ID ${req.params.departmentId}:`,
      err
    );
    res.status(500).json({
      success: false,
      message: "Помилка сервера під час отримання користувачів відділу",
    });
  }
};

/**
 * Отримання всіх користувачів з певною роллю
 * @param {Object} req - Об'єкт запиту Express
 * @param {Object} res - Об'єкт відповіді Express
 */
exports.getUsersByRole = async (req, res) => {
  try {
    ...
    if (!["user", "teamlead", "finance_manager", "admin"].includes(role)) {
      return res.status(400).json({
        success: false,
        message:
          "Недійсна роль. Дозволені значення: user, teamlead, finance_manager, admin",
      });
    }
    ...
    console.error(
      `Помилка отримання користувачів з роллю ${req.params.role}:`,
      err
    );
    res.status(500).json({
      success: false,
      message: "Помилка сервера під час отримання користувачів за роллю",
    });
  }
};

/**
 * Пошук користувачів за іменем, прізвищем або username
 * @param {Object} req - Об'єкт запиту Express
 * @param {Object} res - Об'єкт відповіді Express
 */
exports.searchUsers = async (req, res) => {
  try {
    ...
    if (!query || query.trim() === "") {
      return res.status(400).json({
        success: false,
        message: "Необхідно вказати пошуковий запит",
      });
    }
    ...
    console.error(
      `Помилка пошуку користувачів за запитом "${req.query.query}":`,
      err
    );
    res.status(500).json({
      success: false,
      message: "Помилка сервера під час пошуку користувачів",
    });
  }
};
...
exports.getUserStats = async (req, res) => {
  try {
    ...
    console.error("Помилка отримання статистики користувачів:", err);
    res.status(500).json({
      success: false,
      message: "Помилка сервера під час отримання статистики користувачів",
    });
  }
};
