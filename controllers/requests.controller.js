...
exports.getAllRequests = async (req, res) => {
  try {
    ...
    const params = {
      ...
      financeManagerId: financeManagerId ? parseInt(financeManagerId) : undefined,
      network,
      minAmount,
      maxAmount,
...
exports.updateAgentRefillRequest = async (req, res) => {
  try {
    ...
    if (currentRequest.request_type !== 'agent_refill') {
      return res.status(400).json({
        success: false,
        message: "Цей ендпоінт призначений тільки для заявок типу 'agent_refill'",
      });
    }
    ...
    const nonEditableStatuses = ['completed', 'cancelled', 'rejected_by_finance', 'rejected_by_teamlead'];
    if (nonEditableStatuses.includes(currentRequest.status) && currentUserRole !== 'admin') {
      return res.status(403).json({
        success: false,
    ...
    if (currentUserRole === 'admin') {
      ...
      canEdit = currentRequest.user_id === currentUserId && 
                currentRequest.status === 'pending';
    }
    ...
    const updateData = {};
    const allowedFields = ['amount', 'server', 'wallet_address', 'network', 'transaction_hash', 'fee'];
    ...
    // Оновлення заявки
    const updatedRequest = await requestModel.updateAgentRefillRequest(requestId, updateData);
  ...
  try {
    ...
    allowedFields.forEach(field => {
      if (req.body[field] !== undefined) {
        ...
        else if (['need_transaction_time', 'need_transaction_hash'].includes(field)) {
    ...
    // Оновлення заявки
    const updatedRequest = await requestModel.updateExpenseRequest(requestId, updateData);
...
exports.updateRequestStatus = async (req, res) => {
  try {
    ...
    const statusFlow = {
      ...
      'approved_by_teamlead': ['approved_by_finance', 'rejected_by_finance', 'cancelled'],
    ...
    if (currentUserRole === 'admin') {
      ...
      canChangeStatus = currentRequest.status === 'approved_by_teamlead' && 
      ...
      canChangeStatus = currentRequest.user_id === currentUserId && 
                        ['pending', 'approved_by_teamlead', 'approved_by_finance'].includes(currentRequest.status) && 
                        status === 'cancelled';
    }
    ...
    const updatedRequest = await requestModel.updateRequestStatus(requestId, status, options);
    ...
    console.error(`Помилка оновлення статусу заявки з ID ${req.params.id}:`, err);
    res.status(500).json({
      success: false,
...
exports.deleteRequest = async (req, res) => {
  try {
    ...
    if (!result) {
      return res.status(400).json({
        success: false,
        message: "Не вдалося скасувати заявку. Можливо, вона вже завершена або скасована.",
      });
    }
